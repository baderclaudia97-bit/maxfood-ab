...
export default function BlogPageClient({ locale }: { locale: Locale }) {
  const [selectedCategory, setSelectedCategory] = useState<string>("all");
  const [searchQuery, setSearchQuery] = useState<string>("");

  // Filter posts by category and search query
  const filteredPosts = useMemo(() => {
    let filtered = sampleBlogPosts.filter((post) => post.locale === locale);
    ...
    if (searchQuery) {
      filtered = searchPosts(searchQuery, filtered, locale);
    }
  ...
  const categories = sampleBlogCategories.filter((cat) => cat.locale === locale);
